...
if ($_GET['mode'] == 'ainfo') {
	$a = intval($_GET['a']);
	$tag = mysql_escape_string($_GET['allytag']);
	// Evitamos errores casuales xD
	// query
	$lang['Alliance_information'] = $lang['alliance'];
	if (isset($_GET['allytag'])) {
	...
	if ($ally_description != "") {
		...
		$ally_description = "<tr><th colspan=2 height=100>".$lang['descripcion_new']."</th></tr>";
    ...
    $lang['bewerbung'] = "<tr>
     <th colspan='2'>No se admiten miembros</th>
   </tr>";
   }else{
      $lang['bewerbung'] = "<tr>
     <th>Solicitud</th>
     <th><a href=\"alliance.php?mode=apply&amp;allyid=" . $id . "\">Ingreso en la alianza</a></th>
...
if ($user['ally_id'] == 0) { // Sin alianza
	...
	*/
		...
		*/
			...
			$_POST['atag']  = str_replace($search, $replace, $_POST['atag']);
			$_POST['aname'] = str_replace($search, $replace, $_POST['aname']);
	...
	*/
	...
	if ($mode == 'apply' && $user['ally_request'] == 0) { // solicitudes
      ...
      $alianza = doquery("SELECT * FROM {{table}} WHERE id='{$_GET['allyid']}'", "alliance", true); }
		...
		$allyrow = doquery("SELECT ally_tag,ally_request,ally_request_notallow FROM {{table}} WHERE id='" . intval($_GET['allyid']) . "'", "alliance", true);

if($allyrow['ally_request_notallow']==1){
message("No se permiten solicitudes", "Alianza Cerrada");
}
		if (!$allyrow) {
			message($lang['it_is_not_posible_to_apply'], $lang['it_is_not_posible_to_apply']);
		}
...
elseif ($user['ally_id'] != 0 && $user['ally_request'] == 0) { // Con alianza
	...
	if ($mode == 'circular') { // Correo circular
		...
		if ($sendmail == 1) {
			...
			SendSimpleMessage($u['id'],$user['id'],'','2',$ally['ally_tag'],$user['username'],$user['username'],$_POST['text']);
			}
						/*
	...
	*/
	...
	if ($mode == 'admin' && $edit == 'requests') { // Administrar solicitudes
       ...
       if ($_POST['action'] == $lang['Accept_cand']) {
			if($_GET['diplo']){
doquery("UPDATE {{table}} SET `accept` = '0' WHERE `pact_owner` = '". $_GET['diplo'] ."'", "diplo");
			}else{
			$_POST['text'] = mysql_escape_string(strip_tags($_POST['text']));
		...
		SendSimpleMessage($show,$user['id'],time(),'2',$ally['ally_tag'],"[" . $ally['ally_name'] . "]". $lang['Aplication_acepted_subjet'],$lang['Aplication_hello']."<b>" . $ally['ally_name'] . "</b>".$lang['Aplication_acepted']. $_POST['text']);
		...
		if($_GET['diplo']){
doquery("DELETE FROM {{table}} WHERE `pact_owner` = '". $_GET['diplo'] ."'", "diplo"); 
			$_POST['text'] = mysql_escape_string(strip_tags($_POST['text']));

			doquery("UPDATE {{table}} SET ally_request_text='',ally_request='0',ally_id='0' WHERE id='{$show}'", 'users');
			// Se envia un mensaje avizando...
			SendSimpleMessage($show,$user['id'],time(),'2',$ally['ally_tag'],"[" . $ally['ally_name'] . "]". $lang['Aplication_acepted_subjet'],$lang['Aplication_hello']."<b>" . $ally['ally_name'] . "</b>".$lang['Aplication_rejected']. $_POST['text']);
		...
		while ($r = mysql_fetch_array($query)) {
			...
			$r['link']="show=".$r['id']."&sort=0";
			$r['time'] = date("Y-m-d h:i:s", $r['ally_register_time']);
			$parse['list'] .= parsetemplate($row, $r);
			$i++;
		}
		$query1 = doquery("SELECT * FROM {{table}} WHERE pact_ally_id='{$ally['id']}' AND accept=1", 'diplo');		
		$i = 0;
		while ($r = mysql_fetch_array($query1)) {
		$query3= doquery("SELECT * FROM {{table}} WHERE id='".$r['pact_owner']."'", 'alliance',true);
			if($r['type']==1){
			$type = "<span style='color:green'>[Alianza]</span>";
			}elseif($r['type']==2){
			$type = "<span style='color:blue'>[No Agresion]</span>";
			}elseif($r['type']==3){
			$type = "<span style='color:red'>[Guerra]</span>";
			}
			$type2=$r['type'];
			// recolectamos los datos del que se eligio.
			$t['username'] .= 'Propuesta de '.$query3['ally_name']." ".$type;
			$t2['username']=$query3['ally_name'];
			// la fecha de cuando se envio la solicitud
			$t['link']='diplo='.$r['pact_owner'].'&sort=0';
			$parse['list'] .= parsetemplate($row, $t);
			$i++;
		}
		if ($parse['list'] == '') {
			$parse['list'] = '<tr><th colspan=2>No hay ninguna solicitud mas</th></tr>';
		}
		// Con $show
		if (isset($show) && $show != 0 && $parse['list'] != '') {
			// Los datos de la solicitud
			$s['link']="show=".$show."&sort=0";
		...
		} elseif(isset($_GET['diplo'])) {
			if($type2==1){
			$mensajediplo="una Alianza";
			}elseif($type2 ==2){
			$mensajediplo="un tratado de No Agresion";
			}elseif($type2 ==3){
			$mensajediplo="una Guerra";
			}
				
			$s['link']="diplo=".$_GET['diplo']."&sort=0";
			$s['Request_from'] = str_replace('%s', "<span style='color:red'>".$t2['username']."</span>", "La alianza %s");
			$s['Request_from'] .= str_replace('%r', $mensajediplo, " te propone %r");
			// el formulario
			$s['Accept_cand'] = $lang['Accept_cand'];
			$s['Reject_cand'] = $lang['Reject_cand'];
         $parse['request'] = parsetemplate(gettemplate('alliance_admin_request_form'), $s);
	...
	if ($mode == 'admin' && $edit == 'name') {
		...
		$_POST['newname']  = str_replace($search, $replace, $_POST['newname']);
	...
	if ($mode == 'admin' && $edit == 'tag') {
		...
		$_POST['newtag']  = str_replace($search, $replace, $_POST['newtag']);
	...
	*/
		...
		$cesar=doquery("SELECT id FROM {{table}} WHERE `ally_id`='{$ally['id']}'",'users');
while ($ce = mysql_fetch_array($cesar)) {
   doquery("UPDATE {{table}} SET `ally_name` = '', `ally_id`='0' WHERE `id`='{$ce['id']}'", 'users');
}
...

	{
	 // Default *falta revisar...*
		...
		$request1 = doquery("SELECT * FROM {{table}} WHERE accept='1' AND pact_ally_id ='{$ally['id']}'", 'diplo');
		$request_count1 = mysql_num_rows($request1);
		if ($request_count != 0 || $request_count1 != 0) {
			if ($ally['ally_owner'] == $user['id'] || $ally_ranks[$user['ally_rank_id']-1]['bewerbungen'] != 0)
			$request_total=$request_count + $request_count1;
				$lang['requests'] = "<tr><th>{$lang['Requests']}</th><th><a href=\"alliance.php?mode=admin&edit=requests\">{$request_total} {$lang['XRequests']}</a></th></tr>";
		}
		if ($ally['ally_owner'] != $user['id']) {
