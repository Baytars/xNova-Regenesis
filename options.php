...
$mode = $_GET['mode'];
$activate = $_POST['activator'];

//MOD ACTIVAR CUENTA MEDIANTE MAIL

	if ($activate == "activate1"){
		$to = $user['email_2'];
		$title = "Activacion cuenta tu server";
		$schema = ( isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on' ) ? 'https://' : 'http://';
		$url=dirname($schema . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF']).DIRECTORY_SEPARATOR;
		$siteurlactivationlink = $url.'activate.php?hash='.base64_encode($user['username']).'&stamp='.base64_encode($NewUser['id']).'&mode=options';
		$body ="Bienvenido a tuserver

		Te has registrado con el siguiente nombre de usuario: ". $user['username'] ."

		Para jugar en tuserver es necesario que actives tu cuenta.
		...
		Desde la Administración de tuserver te agradecemos tu confianza";
		$from = "administracion@tuserver";

		mail($to, $title, $body, "From: $from");

		message("Se ha enviado correctamente el link de activacion al siguiente mail: " .$user['email_2'], "Link enviado" );
	}
...
//FIN FIX
			...
			message($lang['succeful_changepass'], $lang['changue_pass'], "login.".$phpEx);
		}
	}
	...
	$parse['dpath'] = $dpath;
	//INICIO FIX AGREGAR  SKINS LOCALES
	$parse['opt_lst_skin_data1']  = "<option value =\"skins/xnova/\">skins/xnova/</option>";
	$parse['opt_lst_skin_data2']  = "<option value =\"skins/EpicBlue/\">skins/EpicBlue/</option>";
	...
	if ($user['activate_status'] != 1){
		$FrameActivate = gettemplate('options_act_acc');
		$bloc['activate_account'] = $lang['activate_account'];
		$bloc['click'] = $lang['click'];
		$parse['opt_activate_frame'] =  parsetemplate($FrameActivate, $bloc);
	}
