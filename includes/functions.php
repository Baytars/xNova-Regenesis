...
function check_urlaubmodus ($user) {
	if ($user['urlaubs_modus'] == 1) {
		message("Usted esta en modo vacaciones!", $title = $user['username'], $dest = "", $time = "3");
	}
}
function check_urlaubmodus_time () {
	...
	if ($game_config['urlaubs_modus_erz'] == 1) {
		...
		if ($user['urlaubs_modus'] == 1 && $urlaub_modus_time_soll > $time_jetzt) {
			...
			message("¡Estás en modo vacaciones!<br>El modo vacaciones dura hasta el $soll_datum $soll_uhrzeit<br> Sólo después de ese período, puede cambiar sus opciones.", "Modo vacaciones");
		}
	}
}
//Text checks
...
//Function based on http://www.desarrolloweb.com/articulos/2373.php
function Check_chars($text)
{
   if (ereg("^[a-zA-Z0-9\-_]{3,20}$", $text)) 
   {
      return true;
   } 
   else 
...
function display ($page, $title = '', $topnav = true, $metatags = '', $AdminPage = false) {
   ...
   if(IsVacationMode($user)&& $title!='Menu' && $AdminPage==false){
    $DisplayPage .= '<center><table width="525" height="25" cellpadding="0" cellspacing="0" border="2" bordercolor="#FF0000"><tr><td><center><font size=2 color=red><b><blink>  '.$lang['VacationMode'].'  </blink></b></font></center></td></tr></table></center>';
}
   $DisplayPage .= "<center>\n". $page ."\n</center>\n";
   ...
   if ($user['authlevel'] == 1 || $user['authlevel'] == 3) {
      if ($game_config['debug'] == 1 && $title!="Menu"){
	   $debug->echo_log();
	   }
   }
...
function  unset_vars( $prefix )
{
	...
	return  $n;  // Núm. de variables encontradas (y "eliminadas")
}
//TODO: Make a better system of update, like update with array or something...
