...
function aks($angreifer_daten, $verteidiger_daten, $angreifer_schiffe, $verteidiger_schiffe, $angreifer_techniken, $verteidiger_techniken, $ressis, $pricelist) {
	...
	$ausgabe = '<html>
<HEAD>
<LINK rel="stylesheet" type="text/css" href=\"css/aks.css\">
  <meta http-equiv="content-type" content="text/html; charset=ISO-8859-1" />
  <TITLE>Informacion del movimiento de flotas</TITLE>
</HEAD>
<BODY>
<div id="overDiv"></div>
<table width="99%">
   <tr>
    <td>
	Las flotas se encontraron el ' . date("m-d H:i:s") . ' combatiendo:
	<br>
';
	...
	foreach ($runde as $rundeid) {
		...
		foreach ($angreifer as $types => $array) {
			if ($types == "schiffe") {
				for ($i = 1; $i <= count($array); $i++) {
					$return_angreifer[$runde_id][$i]['typ'] = "<th>Tipo</th>";
					$return_angreifer[$runde_id][$i]['anz'] = "<th>Anz.</th>";
					$return_angreifer[$runde_id][$i]['waffen'] = "<th>Armas.</th>";
					$return_angreifer[$runde_id][$i]['schilde'] = "<th>Escudos</th>";
					$return_angreifer[$runde_id][$i]['huelle'] = "<th>Blindaje</th>";
				}
		...
		foreach ($verteidiger as $types => $array) {
			if ($types == "schiffe") {
				for ($i = 1; $i <= count($array); $i++) {
					$return_verteidiger[$runde_id][$i]['typ'] = "<th>Tipo</th>";
					$return_verteidiger[$runde_id][$i]['anz'] = "<th>Anz.</th>";
					$return_verteidiger[$runde_id][$i]['waffen'] = "<th>Armas.</th>";
					$return_verteidiger[$runde_id][$i]['schilde'] = "<th>Escudos</th>";
					$return_verteidiger[$runde_id][$i]['huelle'] = "<th>Blindaje</th>";
				}
	...
	<table border=1 width=100%>
		...
		for ($i = 1; $i <= count($return_angreifer[$runde_id]); $i++) {
			...
			<th>
				<br>
				<center>
					Atacante " . $angreifer_daten[$i]['name'] . " (" . $angreifer_daten[$i]['koords'] . ")";
			if ($runde_id == 1) {
				$ausgabe .= "
					<br>
					Armas: " . ($angreifer_techniken[$i]['waffen'] * 10) . "% Escudos: " . ($angreifer_techniken[$i]['schilde'] * 10) . "% Blindaje: " . ($angreifer_techniken[$i]['panzerung'] * 10) . "%";
			}
	...
	<table border=1 width=100%>
		...
		for ($i = 1; $i <= count($return_verteidiger[$runde_id]); $i++) {
			...
			<th>
				<br>
				<center>
					Defensor " . $verteidiger_daten[$i]['name'] . " (" . $verteidiger_daten[$i]['koords'] . ")";
			if ($runde_id == 1) {
				$ausgabe .= "
					<br>
					Armas: " . ($verteidiger_techniken[$i]['waffen'] * 10) . "% Escudos: " . ($verteidiger_techniken[$i]['schilde'] * 10) . "% Blindaje: " . ($verteidiger_techniken[$i]['panzerung'] * 10) . "%";
			}
	...
	<br>
	<center>
		El atacante, dispara con un total de <b>" . pretty_number($runde[$runde_id]['angreifer']['schuesse']) . "</b> veces, con una furza de <b>" . pretty_number($runde[$runde_id]['angreifer']['schusskraft']) . "</b> contra el defensor.
		Los escudos del defensor absorben <b>" . pretty_number($runde[$runde_id]['verteidiger']['schilde_weg']) . "</b> disparos.
		<br>
		El defensor, dispara con un total de <b>" . pretty_number($runde[$runde_id]['verteidiger']['schuesse']) . "</b> veces, con una fuerza de <b>" . pretty_number($runde[$runde_id]['verteidiger']['schusskraft']) . "</b> contra el atacante.
		Los escudos atacante absorben <b>" . pretty_number($runde[$runde_id]['angreifer']['schilde_weg']) . "</b> disparos
	</center>";
		} else {
	...
	if ($ausgang == "angreifer") {
		$ausgabe .= "
	<p style="color:red; font-size:x-large;">Gana el atacante!</p>
	<br>
			";
		...
		$ausgabe .= "
	Capturando<br><b>" . pretty_number($angreifer_beute_metall) . "</b> Metal, <b>" . pretty_number($angreifer_beute_kristall) . "</b> Cristal y <b>" . pretty_number($angreifer_beute_deuterium) . "</b> Deuterio
	<br>";
	} elseif ($angreifer_komplettvernichtet) {
		$ausgang = "verteidiger";
	} else {
	...
	<p>
	<br>
	El atacante tiene un total de " . pretty_number($verluste_angreifer) . " unidades perdidas.
	<br>
	El defensor tiene un total de " . pretty_number($verluste_verteidiger) . " unidades perdidas.
	<br>
	En estas coordenadas flotan <b>" . pretty_number($metall) . "</b> Metal y <b>" . pretty_number($kristall) . "</b> Cristal.
	<br>" . $repairtext . " unidades podran ser reparadas.
	<br><br>
	La batalla duro <b>" . $totaltime . "</b> Segundos
    </td>
