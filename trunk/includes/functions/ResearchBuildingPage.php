...
function ResearchBuildingPage (&$CurrentPlanet, $CurrentUser, $InResearch, $ThePlanet) {
	...
	if (!CheckLabSettingsInQueue ( $CurrentPlanet )) {
	  ...
	  if( $intergal_lab < "1" ) {
         $lablevel = $CurrentPlanet[$resource['31']];
      } else {
      $limite = $intergal_lab+1;
         $inves = doquery("SELECT SUM(laboratory) as laboratorio FROM {{table}} WHERE id_owner='".$CurrentUser['id']."' order by id asc limit $limite", 'planets');
         $investiga = mysql_fetch_array($inves);
         $lablevel = $investiga['laboratorio'];
      }

	// Boucle d'interpretation des eventuelles commandes
	if (isset($_GET['cmd']) && !$game_config['game_disable']==1) {
		$TheCommand = $_GET['cmd'];
		$Techno     = $_GET['tech'];
				...
				switch($TheCommand){
					...
					case 'search':
						if ( IsTechnologieAccessible($CurrentUser, $WorkingPlanet, $Techno) &&
							...
							$WorkingPlanet["b_tech"]      = time() + GetBuildingTime($CurrentUser, $WorkingPlanet, $Techno, $lablevel);
	...
	foreach($lang['tech'] as $Tech => $TechName) {
		if ($Tech > 105 && $Tech <= 199) {
			if ( IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Tech)) {
				...
				$SearchTime              = GetBuildingTime($CurrentUser, $CurrentPlanet, $Tech,$lablevel);
