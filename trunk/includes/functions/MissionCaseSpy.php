...
function MissionCaseSpy ( $FleetRow ) {
	...
	if ($FleetRow['fleet_start_time'] <= time()) {
		...
		$CurrentSpyLvl       = $CurrentUser['spy_tech'] + ($TargetUser['rpg_espion']*5);
...
doquery("DELETE FROM {{table}} WHERE `fleet_id` = '". $FleetRow['fleet_id']."' AND  `fleet_mission`=6;", 'fleets');				//Elimina las sondas si se pierden en el espionaje	
						}
						$AttackLink = "<center>";
