...
function DefensesBuildingPage ( &$CurrentPlanet, $CurrentUser ) {
 	global $lang, $resource, $phpEx, $dpath, $game_config, $_POST;
	...
	if (isset($_POST['fmenge'])) {
		...
		foreach($_POST['fmenge'] as $Element => $Count) {
			...
			if ($Count != 0 &&!$game_config['game_disable']==1) {
	...
	foreach($lang['tech'] as $Element => $ElementName) {
		if ($Element > 400 && $Element <= 599) {
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
				...
				//FIN FIX CUPULAS
				if($game_config['game_disable']==1 || $game_config['mode_maintenance']==1){
					$TabIndex++;
					$PageTable .="<font color=red>En Mantenimiento</font>";
					$PageTable .= "</th>";
				}elseif ( !$BuildIt ) {
