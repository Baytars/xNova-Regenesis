...
function BuildFleetEventTable ( $FleetRow, $Status, $Owner, $Label, $Record ) {
	...
	$FleetPlanet    = doquery("SELECT p.name as atacante , p1.name as atacado FROM {{table}}planets as p, {{table}}planets as p1 WHERE (p.galaxy = '".$FleetRow['fleet_start_galaxy']."' AND p.system = '".$FleetRow['fleet_start_system']."' AND p.planet = '".$FleetRow['fleet_start_planet']."' AND p.planet_type = '".$FleetRow['fleet_start_type']."') AND ( p1.galaxy = '".$FleetRow['fleet_end_galaxy']."' AND p1.system = '".$FleetRow['fleet_end_system']."' AND p1.planet = '".$FleetRow['fleet_end_planet']."' AND p1.planet_type = '".$FleetRow['fleet_end_type']."') ;", '', true);
   ...
   if ($MissionType == 10){
	...
	$bloc['fleet_time']   = gmdate("H:i:s", $Time + 2 * 60 * 60);
