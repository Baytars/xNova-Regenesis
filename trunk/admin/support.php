...
if ($user['authlevel'] >= 1)
{
	...
	else
	{
		...
		$query2 = doquery("SELECT s.*, u.username as username , u.id FROM {{table}}supp as s, {{table}}users as u  WHERE `ID` = '".$_GET['ticket']."' AND u.id=s.player_id ", "");
