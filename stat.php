...
if ($who == 2) 
{
	...
	$start++;
	$parse['stat_date']   = $game_config['stat_last_update'];
	$parse['stat_values'] = "";
...
else
{
	...
	$query = doquery($stats_sql, '');
	$start++;
	$parse['stat_date']   =  date("d M Y - H:i:s",$game_config['stat_last_update']);
	$parse['stat_values'] = "";
	while ($StatRow = mysql_fetch_assoc($query)) 
	{
		...
		{
			$parse['player_alliance'] = "<a href=\"alliance.php?mode=ainfo&a=".$StatRow['ally_id']."\"><font color=\"#33CCFF\">".$StatRow['ally_name']."</font></a>";
		} 
		else 
		{
			$parse['player_alliance'] = "<a href=\"alliance.php?mode=ainfo&a=".$StatRow['ally_id']."\">".$StatRow['ally_name']."</a>";
		}
		// FIN FIX
