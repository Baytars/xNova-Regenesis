...
class Zend_Locale_Data {
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            $filename =ROOT.'/languages/' . $locale . '.xml';
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        switch(strtolower($path)) {
            ...
            case 'country':
            case 'territory':
                $_path = '/ldml/localeDisplayNames/territories/territory';
                
                if ( !is_numeric( $value ) ) {
                    $_path .= "[child::text()='{$value}' or @type='{$value}']";
                }

                $temp = self::_getFile($locale, $_path, 'type');
                if ($value === 1) {
            ...
            case 'region':
                $value = '[@id=\''.$value.'\' or child::text()=\''.$value.'\']';
                
                $temp = self::_getFile($locale, '/ldml/localeDisplayNames/regions/region'.$value, 'id');
            break;

            case 'city':
                   $temp = self::_getFile($locale, '/ldml/localeDisplayNames/cities/city'. (($value) ? '[child::text() = \''.$value.'\']' : ''), 'id');
                   break;
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        switch ( strtolower( $path ) ) {
            ...
            case 'region':
                $value = is_numeric($value) ? '[@id=\''.$value.'\']' : '[child::text()=\''.$value.'\']';

                $temp = self::_getFile($locale, '/ldml/localeDisplayNames/regions/region'.$value, 'id');
            break;

            case 'city':
                $temp = self::_getFile($locale, '/ldml/localeDisplayNames/cities/city'. (!is_numeric($value) ? '[child::text() = \''.$value.'\']' : '[@id=\''.$value.'\']'), 'id' );
            break;

            case 'country':
            case 'territory':
                $temp = self::_getFile($locale, '/ldml/localeDisplayNames/territories/territory[@type=\'' . $value . '\' or child::text()=\''.$value.'\']', 'type');
            break;
